...
def newFiles():
	try:
		with open('SavedList_seq.pkl', 'rb') as f:
	...
	with open('SavedList_seq.pkl', 'wb') as f:
...
def make_indices(filenames):
	...
	for filename in filenames:
		...
		boolean=filename.endswith(extensionsToCheck)
		#print(boolean)
		if(boolean):
			try:
				termlists=convert_pdf_to_txt('example.pdf')
			except:
				continue
		else:
